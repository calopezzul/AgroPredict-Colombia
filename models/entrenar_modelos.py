...
import joblib
from utils.limpiar import leer_csv_seguro, limpiar_dataframe, limpiar_texto_colombia
...
df_lr = leer_csv_seguro(os.path.join(BASE, 'suelos_regresion_lineal.csv'))
...
df_log = leer_csv_seguro(os.path.join(BASE, 'suelos_regresion_logistica.csv'))
...
df_tree = leer_csv_seguro(os.path.join(BASE, 'suelos_arboles_decision.csv'))
...
df_raw = leer_csv_seguro(csv_raw)
...
df_raw['depto'] = df_raw['depto'].apply(limpiar_texto_colombia)
